...
query = '?query=(status.name="Open" AND dueDate < "2020-04-10*")'
...
for loan in loans:
    ...
    loan_to_put['dueDate'] = '2020-04-10T22:59:59.000+0000'
    print(loan_to_put['dueDate'])
    try:
        put_loan(folio_client, loan_to_put)
        print(f"PUT /circulation/loans/{loan_to_put['id']}\t{json.dumps(loan_to_put)}\t{loan}]")
        patron = folio_client.folio_get_single_object(f"/users/{loan['userId']}")
        patron_emails.add(patron['personal']['email'])
    except Exception as ee:
        print(f"ERROR! PUT /circulation/loans/{loan_to_put['id']}\t{json.dumps(loan_to_put)}\t{loan}]")
        print(ee)
        traceback.print_exc()
        raise ee
    i+=1
